  ...
  function agregarPorCampo(dados, chaveFn) {
    const agrupado = {};
    dados.forEach((item) => {
      const chave = chaveFn(item);
      if (!agrupado[chave]) agrupado[chave] = { nome: chave, total: 0, lancamentos: 0 };
      agrupado[chave].total += item.valor;
      agrupado[chave].lancamentos += 1;
    });
  ...
  function gerarTabelaRelatorio(titulo, colunas, linhas) {
    return `
  ...
  function exportarRelatorioGerencial() {
    const dados = obterDadosFiltradosAtuais();
    if (!dados.length) {
      atualizarStatusSalvamento('Nao ha dados filtrados para gerar o relatorio.', 'error');
      return;
    }

    const total = dados.reduce((soma, item) => soma + item.valor, 0);
    const porResponsavel = agregarPorCampo(dados, (item) => item.supervisor).slice(0, 15);
    const porUnidade = agregarPorCampo(dados, (item) => item.unidade);
    const porCidade = agregarPorCampo(dados, (item) => item.cidade).slice(0, 15);
    const porCombustivel = agregarPorCampo(dados, (item) => item.combustivel);
    const porVeiculo = agregarPorCampo(dados, (item) => `${item.placa} - ${item.veiculo}`).slice(0, 15);
    const porMes = agregarPorCampo(dados, (item) => item.mes);
    const mesMaiorGasto = porMes[0] || null;
    const destaqueSprinter = porVeiculo.find((item) => item.nome.toLowerCase().includes('sprinter'));
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Relatorio Gerencial de Combustivel</title>
        <style>
          body { font-family: Arial, sans-serif; margin: 32px; color: #17324a; }
          h1 { margin: 0 0 8px; color: #073954; }
          h2 { margin: 28px 0 12px; color: #0a5678; }
          .meta { color: #60758a; margin-bottom: 22px; }
          .summary { display: grid; grid-template-columns: repeat(4, minmax(140px, 1fr)); gap: 12px; margin-bottom: 28px; }
          .summary div { border: 1px solid #d9e4ec; border-radius: 12px; padding: 14px; background: #f7fafc; }
          .summary span { display: block; color: #60758a; font-size: 12px; margin-bottom: 6px; }
          .summary strong { font-size: 24px; color: #073954; }
          table { width: 100%; border-collapse: collapse; margin-top: 10px; }
          th, td { border-bottom: 1px solid #d9e4ec; padding: 10px 8px; text-align: left; font-size: 14px; }
          th { color: #60758a; }
          .highlight { padding: 14px 16px; border-radius: 12px; background: #edf6fb; border: 1px solid #cfe2ee; margin: 18px 0; }
        </style>
      </head>
      <body>
        <h1>Relatorio Gerencial de Combustivel</h1>
        <div class="meta">Gerado em ${new Date().toLocaleString('pt-BR')} | Periodo: ${mesInicio.value || 'inicio livre'} ate ${mesFim.value || 'fim livre'}</div>
        <div class="summary">
          <div><span>Total gasto</span><strong>${formatarBRL(total)}</strong></div>
          <div><span>Responsaveis</span><strong>${new Set(dados.map((item) => item.supervisor)).size}</strong></div>
          <div><span>Placas</span><strong>${new Set(dados.map((item) => item.placa)).size}</strong></div>
          <div><span>Lancamentos</span><strong>${dados.length}</strong></div>
        </div>
        ${mesMaiorGasto ? `<div class="highlight"><strong>Mes com maior gasto:</strong> ${mesMaiorGasto.nome} com ${formatarBRL(mesMaiorGasto.total)}</div>` : ''}
        ${destaqueSprinter ? `<div class="highlight"><strong>Sprinter:</strong> ${destaqueSprinter.nome} gastou ${formatarBRL(destaqueSprinter.total)}.</div>` : ''}
        ${gerarTabelaRelatorio('Total por responsavel', ['Responsavel', 'Total', 'Lancamentos'], porResponsavel.map((linha) => `<tr><td>${linha.nome}</td><td>${formatarBRL(linha.total)}</td><td>${linha.lancamentos}</td></tr>`))}
        ${gerarTabelaRelatorio('Total por unidade / regiao', ['Unidade', 'Total', 'Lancamentos'], porUnidade.map((linha) => `<tr><td>${linha.nome}</td><td>${formatarBRL(linha.total)}</td><td>${linha.lancamentos}</td></tr>`))}
        ${gerarTabelaRelatorio('Resumo por combustivel', ['Combustivel', 'Total', 'Lancamentos'], porCombustivel.map((linha) => `<tr><td>${linha.nome}</td><td>${formatarBRL(linha.total)}</td><td>${linha.lancamentos}</td></tr>`))}
        ${gerarTabelaRelatorio('Top veiculos / placas', ['Veiculo', 'Total', 'Lancamentos'], porVeiculo.map((linha) => `<tr><td>${linha.nome}</td><td>${formatarBRL(linha.total)}</td><td>${linha.lancamentos}</td></tr>`))}
        ${gerarTabelaRelatorio('Top cidades', ['Cidade', 'Total', 'Lancamentos'], porCidade.map((linha) => `<tr><td>${linha.nome}</td><td>${formatarBRL(linha.total)}</td><td>${linha.lancamentos}</td></tr>`))}
      </body>
      </html>
    `;
    ...
    link.href = url;
    link.download = `relatorio-gerencial-combustivel-${new Date().toISOString().slice(0, 10)}.html`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    atualizarStatusSalvamento('Relatorio gerencial exportado com sucesso.', 'success');
  }
  ...
  function aplicarFiltros() {
    if (!baseDados.length) return;
    const filtrado = obterDadosFiltradosAtuais();
  ...
  document.getElementById('exportarFiltrado').addEventListener('click', () => {
    ...
    const filtrado = obterDadosFiltradosAtuais().map((item) => ({
  ...
  document.getElementById('exportarRelatorio').addEventListener('click', exportarRelatorioGerencial);
